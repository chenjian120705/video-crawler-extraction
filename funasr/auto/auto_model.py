...
class AutoModel:
    ...
    def build_model(**kwargs):
        ...
        logging.info(
            "正在构建模型结构: %s (device=%s, class=%s)",
            kwargs.get("model"),
            device,
            model_class.__name__,
        )
        model = model_class(**model_conf)
        logging.info("模型结构构建完成: %s", kwargs.get("model"))
    ...
    def generate(self, input, input_len=None, progress_callback=None, **cfg):
        ...
        if self.vad_model is None:
            logging.info("开始语音推理（整段识别，无 VAD 分段）")
            ...
            logging.info("开始语音推理（VAD 分段 + ASR 识别）")
    ...
    def inference(
        ...
        **cfg,
        ...
        disable_pbar = kwargs.get("disable_pbar", self.kwargs.get("disable_pbar", False))
        pbar = (
    ...
    def inference_with_vad(self, input, input_len=None, **cfg):
        ...
        logging.info("开始 VAD 语音分段…")
        beg_vad = time.time()
        res = self.inference(
            input, input_len=input_len, model=self.vad_model, kwargs=self.vad_kwargs, **cfg
        )
        end_vad = time.time()
        vad_segments_total = sum(len(item.get("value", [])) for item in res)
        logging.info(
            "VAD 分段完成: 样本 %d 个, 语音片段 %d 段, 耗时 %.1fs",
            len(res),
            vad_segments_total,
            end_vad - beg_vad,
        )
        ...
        end_total = time.time()
        merged_text_len = sum(len(item.get("text", "")) for item in results_ret_list)
        logging.info(
            "ASR 分段识别全部完成: 样本 %d 个, 合并文本约 %d 字, 总耗时 %.1fs",
            len(results_ret_list),
            merged_text_len,
            end_total - beg_total,
        )
